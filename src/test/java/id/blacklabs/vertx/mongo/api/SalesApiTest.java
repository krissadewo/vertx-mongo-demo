...
public class SalesApiTest extends BaseTest {
    ...
    Logger logger = LoggerFactory.getLogger(SalesApiTest.class);

    @Test
    void save_sales(Vertx vertx, VertxTestContext testContext) {
        WebClient webClient = WebClient.create(vertx);
        Checkpoint requestCheckpoint = testContext.checkpoint(1);

        webClient.post(8888, "localhost", "/sales")
            .as(BodyCodec.json(Object.class))
            .sendJsonObject(JsonObject.mapFrom(new DummyData().sales()), testContext.succeeding(response -> testContext.verify(() -> {
    ...
    @Test
    void find_sales(Vertx vertx, VertxTestContext testContext) {
        WebClient webClient = WebClient.create(vertx);
        Checkpoint requestCheckpoint = testContext.checkpoint(1);
